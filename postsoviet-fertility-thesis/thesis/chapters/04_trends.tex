...
In 2000 the fourteen countries in this study ranged from 1.11 children per woman in Ukraine to 3.49 in Tajikistan. Twenty-three years later the range had not narrowed. Ukraine had fallen to 0.98, one of the lowest in the region, while Uzbekistan had risen to 3.50. The endpoints are almost identical to where they started; what changed is which countries occupy them and how they got there.
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{fig1_all_countries}
...
Figure~\ref{fig:bloc-means} tracks the two bloc means with the country range around each. The Central Asian mean rose from 2.67 in 2000 to 3.10 in 2023, an increase of 0.43 children per woman. The rest of the post-Soviet space rose too, from 1.38 to a peak of 1.77 in 2016, and then gave nearly all of it back, ending at 1.44.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.92\textwidth]{fig2_bloc_means}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.88\textwidth]{fig4_gap}
...
The post-2016 dispersion slope is positive in all three samples: $+0.0195$ in the full sample, $+0.0237$ using all fourteen countries over 2000--2019 with no projected values anywhere, and $+0.0261$ using the ten countries with a non-projected observation in every year from 2017 to 2022. Rather than casting doubt on the divergence result, the exercise strengthens it: the finding is not an artefact of projected endpoints, and it is not an artefact of a shrinking, low-fertility-biased country set. The $\beta$ estimate is less stable, at $+0.0013$, $-0.0085$ and $+0.0052$ across the three samples, but nothing rests on its sign, and all three are consistent with the conclusion that there is no detectable catch-up. Full results appear in Appendix~\ref{app:projection-check}.

A second check separates the two sides of the bifurcation. The 2022 invasion of Ukraine was a regional shock rather than a Ukrainian one, so the dispersion trends are refitted with 2022 and 2023 removed for all fourteen countries. Central Asian internal convergence is unaffected and slightly stronger without those years, moving from $-0.0075$ to $-0.0082$ per year and remaining significant at any conventional level. The divergence among the other ten countries does not hold up as well: its slope falls from $+0.0016$ to $+0.0007$ per year and no longer reaches conventional significance ($p = 0.088$). The direction is unchanged in both cases, but the evidence is asymmetric, and the bifurcation described above rests mainly on the Central Asian side of it. The corresponding check on the regression estimates appears in Section~\ref{sec:robustness}.
