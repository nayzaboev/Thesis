...
All three columns are estimated on the 315 observations for which the controls are complete, so that the raw and conditional estimates are comparable. Dropping those twenty-one observations costs nothing: on the full 336-observation panel the raw difference is $+1.324$ rather than $+1.314$.

So the four covariates account for about twenty-seven per cent of the raw difference. Nearly three quarters of it remains. It is worth pausing on what a residual difference of $0.96$ children per woman actually amounts to. The mean fertility rate of the ten non-Central-Asian countries moved between 1.34 and 1.77 over the twenty-four years, a total range of 0.43. The part of the Central Asian difference that survives economic controls is therefore more than twice the entire range of variation the comparison bloc displayed across the whole period, including the recovery of the 2000s and the decline after 2016.

Two features of the control coefficients deserve comment. First, not one of the four is distinguishable from zero at conventional levels in either column. Log GDP per capita is essentially zero ($+0.009$, interval $[-0.244, +0.263]$). Urbanisation has the expected negative sign but its interval crosses zero. Remittances and under-five mortality are small and imprecise. The controls are jointly doing work --- $R^2$ rises from 0.842 to 0.908 when they enter --- but no individual variable can be credited with it.
...
The answer is essentially no. In the two-way fixed-effects model none of the four coefficients comes close to conventional significance. In first differences, under-five mortality is significant ($-0.008$, $p = 0.013$) and log GDP is marginal ($p = 0.078$) --- but adding year effects removes both, taking under-five mortality to $-0.0001$ with a p-value of 0.968. A coefficient that reverses this completely on the inclusion of common year effects cannot be interpreted, and this thesis does not interpret it.

The choice between the levels and differenced specifications is not arbitrary. An AR(1) regression on the fixed-effects residuals returns a coefficient of $0.948$. Residuals that persistent mean the levels model is at serious risk of picking up shared trending behaviour rather than a relationship, so the first-difference results are treated as the more cautious evidence and the levels model as descriptive. The differenced specification explains 5.6 per cent of the variation in fertility changes without year effects and 25.7 per cent with them, which tells its own story: most of what moves fertility from year to year in these countries is common across them, not driven by their individual macroeconomic trajectories.

It matters to be clear about what this does and does not show. It does not show that economic conditions are unimportant for fertility. It shows that these four indicators, at annual frequency, in this fourteen-country panel, over twenty-four years, do not have stable measurable within-country associations with the period fertility rate. Annual macro series are noisy, fertility decisions respond with lags of uncertain length, and a first-differenced specification discards precisely the long-run level information that the between-country layer exploits. What the two layers together establish is where the Central Asian difference lives: it is a persistent difference in levels between countries, not something that can be recovered from the year-to-year co-movement of fertility and macroeconomic conditions inside them.
...
Four checks were run on the controlled difference, and all are reported in full in Appendix~\ref{app:projection-check}. Dropping one country at a time moves it within $[+0.778, +1.081]$, the extremes being Moldova and Kazakhstan; it stays positive and significant in all fourteen re-estimations. Four targeted exclusions --- Ukraine's war years, Uzbekistan's post-2017 surge, Tajikistan entirely and Azerbaijan entirely --- give $+0.965$, $+0.938$, $+0.951$ and $+1.054$, so the result is not an artefact of any one episode or borderline case. A country-mean regression with HC3 errors, which avoids cluster asymptotics altogether, returns $+1.324$ for the raw difference. And a permutation test across all $\binom{14}{4} = 1{,}001$ possible four-country groupings finds the observed Central Asian grouping the single most extreme of the 1{,}001, giving a one-sided p-value of 0.001 --- a test of whether the grouping is unusual, not of whether the region causes anything.

\subsection{What this chapter establishes}
\label{sec:premium-summary}

The Central Asian fertility difference is large, it is not an artefact of the specification, and it does not depend on any single country or episode. Approximately a quarter of it moves together with income, urbanisation, remittances and child survival. The remaining $0.96$ children per woman does not. Year-to-year variation within countries offers no stable evidence on what accounts for it, which locates the difference firmly in persistent between-country variation. And a country-level cross-section can show that the surviving difference is correlated with markers of religious-cultural and nuptiality regimes while being unable to separate those markers from regional identity itself.
