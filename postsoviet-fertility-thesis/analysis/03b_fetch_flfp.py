...
df = raw.reset_index().melt(id_vars="economy", var_name="year", value_name="flfp_pct")
df["year"] = df["year"].str.replace("YR", "", regex=False).astype(int)
df["country"] = df["economy"].map(name_map)
df = df[["country", "year", "flfp_pct"]]
df = df.sort_values(["country", "year"]).reset_index(drop=True)

os.makedirs("data/raw", exist_ok=True)
df.to_csv("data/raw/raw_wb_flfp.csv", index=False)
print(f"Saved {len(df)} rows to data/raw/raw_wb_flfp.csv")
